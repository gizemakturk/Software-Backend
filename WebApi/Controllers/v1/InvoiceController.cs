...
using Application.Features.Invoices.Commands.DeleteInvoiceById;
...
namespace WebApi.Controllers.v1
{
    ...
    public class InvoiceController : BaseApiController
    {
        // GET: api/<controller>
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] GetAllInvoicesParameter filter)
        {
        ...
        [HttpPut("{id}")]
        //[Authorize]
        public async Task<IActionResult> Put(int id, UpdateInvoiceCommand command)
        {
            if (id != command.Id)
            {
                return BadRequest();
        ...
        {
            return Ok(await Mediator.Send(new DeleteInvoiceByIdCommand { Id = id }));
        }
    }
}
